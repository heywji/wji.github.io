...
诚实记录：NAS 上还挂着一个 OpenVPN 的 `route-up` 钩子脚本，内容正是上面那条回程路由命令。但在故障发生时，**它加的路由并不在表里**。

OpenVPN 在故障前重启过，`route-up` 那行也在配置里，理论上应该执行了。到底是**它压根没触发**，还是**触发了但路由后来被冲掉**，我没能确定——根分区是 `noatime` 挂载的，`atime` 这条非侵入式线索用不了；日志里也没有记录。手工执行该脚本是正常的。

要定论只有一个办法：重启 VPN 服务，然后立刻看 table 100。代价是掐断当前隧道。

**但不管答案是哪个，工程结论是同一个：** VPN 服务重启会销毁重建 tun0，而 Linux 内核在网卡消失时会清掉所有引用它的路由——**自定义表也不例外**。所以 table 100 里那条 `dev tun0` 的回程路由，在每次 VPN 服务重启后都会消失，而"开机跑一次"的任务不会再补。

群晖的「触发的任务」只有开机/关机两种事件，没有"服务启动"钩子。所以要么依赖 OpenVPN 自己的 `route-up`（可靠性存疑），要么加一个定期任务轮询兜底，要么接受"每次改完 VPN 设置手动跑一次"。
...
这个故障的技术根因只有一行路由，但它能活下来，靠的是三层叠加：
